    ...
    featureIds.forEach(function (featureId) {
        ...
        if (ns[0] === "blackberry") {
            for (i = 1; i < ns.length; i++) {
                ...
                // If "objects" or "children" field is not existing, don't continue
                if (originalExtensionObject[key] === undefined) {
                    break;
                }

                originalExtensionObject = originalExtensionObject[key][ns[i]];

                // If the namespace is not existing, don't continue
                if (originalExtensionObject === undefined) {
                    break;
                }
...
FeatureManager.prototype.getServerPathForFeature = function (featureId) {
    var ns = featureId.split("."),
        feature;

    if (ns[0] === "blackberry") {
        feature = this._libraries.objects[ns.slice(1)[0]];

        if (feature) {
            ns.slice(2).forEach(function (e) {
                if (feature.children && feature.children.hasOwnProperty(e)) {
                    feature = feature.children[e];
                } else {
                    feature = null;
                }
            });
    ...
    return feature && feature.serverPath;
};
