...
function isMatch(access, requestURI) {
    ...
    if (isLocalAccess(access)) {
        if (util.isLocalURI(requestURI) || util.isFileURI(requestURI)) {
...
function getAccessByUrl(url) {
    ...
    if (util.isAbsoluteURI(requestURI)) {
        ...
        // Start with the full authority path and check if a Access set exists for that path
        ...
        if (requestURI.path === "") {
            ...
            fetchedAccess = folderAccess.getAccess(requestURI.path + requestURI.query);
        }            
