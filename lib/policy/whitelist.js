...
function authorityCheck(scheme, authority) {
    ...
    if ((portIndex = authority.indexOf(":")) > -1) {
        ...
        if (!_authorityCollection.hasOwnProperty(scheme + "://" + authority)) {
...
function getAccessByUrl(url) {
    ...
    if (util.isAbsoluteURI(requestURI)) {
        ...
        if (fetchedAccess) {
            return fetchedAccess;
        } else if (_localAccess && isMatch(_localAccess, requestURI)) {
...
_self = {
     ...
     * @param {Array} accessList - access list from config
     * @param {boolean} hasGlobalAccess - if app can access any domains
     */
    ...
    /**
     * Determine if a specified feature can be used in a given URL
     * @param {String} url - request URL
     * @param {String} feature - id of the feature
     * @return true if the specified feature can be used for this URL
     */
    isFeatureAllowed : function (url, feature) {
        var access = getAccessByUrl(url), i;
    ...
    /**
     * Determine if a specified URL can be accessed
     * @param {String} url - request URL
     * @return true if the URL can be accessed
     */
