 ...
 */
var ConfigConstants = require("../config/ConfigConstants.js"),
    WidgetWebFolderAccess = require("./folderAccess.js").WidgetWebFolderAccess,
    util = require("../utils.js"); 
var _accessList, _authorityCollection, _localAccess, _self;

function initializeAuthCollection() {
    var access, folderAccess, currentURI;

    if (!_authorityCollection) {
        for (access in _accessList) {
            if (access.uri === ConfigConstants.WIDGET_LOCAL_DOMAIN) {
                _localAccess = access;
            } else {
...
_self = {
    initialize : function (accessList) {
        _accessList = accessList;
    },
    
    isFeatureAllowed : function (url, feature) {
        var access = getAccessByUrl(url), features, f;
