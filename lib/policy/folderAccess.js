...
function getAccessRecursively(folderPath, pathLength) {
    if (folderPath === "") {
        return null;
    } else if (_pathCollection.hasOwnProperty(folderPath)) {
        // Check folder path if an entry exists for the full path
        return _pathCollection[folderPath];
    } else {
        // Truncate the end portion of the path and try again
        var newPathLength, newPath;

        newPathLength = Math.min(_maxPathLength, pathLength - 1);
        newPath = getPath(folderPath, newPathLength);

        return getAccessRecursively(newPath, newPathLength);
    }
}
...
WebFolderAccess.prototype.addAccess = function (folderPath, access) {
    ...
    _pathCollection[folderPath] = access;
...
WebFolderAccess.prototype.getAccess = function (folderPath) {   
    ...
    return getAccessRecursively(folderPath, depth);
};
