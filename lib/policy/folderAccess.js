...
/**
 * Helper class to store a map of access objects indexed by paths
 */
...
/**
 * Adds accses to the map by with folder path as a key
 * @param folderPath - folder path must not include the scheme or the host
 * @param access
 */
...
/**
 * Retrieves the access assigned to the folder path, if it exists
 * @param folderPath - folder path must not include the scheme or the host
 * @return the matching access or null
 */
...
/**
 * Retrieves the access from path collection, taking query string wildcard
 * into consideration
 * @param folderPath - folder path must not include the scheme or the host
 * @return the matching access or null
 */
WebFolderAccess.prototype.fetchAccess = function (folderPath) {
 ...
 * Retrieves the access assigned to the folder path, if it exists
 * @param folderPath - folder path must not include the scheme or the host
 * @param pathLength - depth of the path
 * @return the matching access or null
 */
WebFolderAccess.prototype.getAccessRecursively = function (folderPath, pathLength) {
    var fetchedAccess,
        newPathLength,
	newPath;
    ...
    if (folderPath === "") {
        return null;
    } else if (!!(fetchedAccess = this.fetchAccess(folderPath))) {
