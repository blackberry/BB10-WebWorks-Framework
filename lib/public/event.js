...
var _handlers = {},
    ...
    windowObj = isEmpty(require.main.exports) ? require("./window") : null;

module.exports = {
    on: function (featureId, name, cb, success, fail) {
        if (featureId && name && typeof cb === "function") {
            var win = inNode() ? windowObj.window() : window; // HACK
            ...
            win.webworks.exec(success, fail, featureId, "on", {"eventName": name});
        }
    },
    ...
    remove: function (featureId, name, cb, success, fail) {
        if (featureId && name && typeof cb === "function") {
            if (_handlers.hasOwnProperty(name)) {
                ...
                win = inNode() ? windowObj.window() : window; // HACK
                ...
                if (found >= 0) {
                    ...
                    if (_handlers[name].length === 0) {
                        ...
                        win.webworks.exec(success, fail, featureId, "remove", {"eventName": name});
                    }
                }
            }
        }
