...
var _self = {
    onListFeatures: function (request) {
        ...
        var featureIds = whitelist.getFeaturesForUrl(_getOrigin(request)),
            extensionObjects = featureMgr.getExtensionsObjectForFeatures(featureIds);
    ...
    onExecFeature: function (request) {
        ...
        if (whitelist.isFeatureAllowed(_getOrigin(request), featureId)) {
            feature = require(featureMgr.getServerPathForFeature(featureId));
