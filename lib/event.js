...
module.exports = {
    trigger: function (actionEvent) {
        ...
        if (_handlers.hasOwnProperty(actionEvent)) {
            _handlers[actionEvent].forEach(function (webview) {
                webview.executeJavaScript("webworks.event.trigger('" + actionEvent + "', '" + escape(encodeURIComponent(JSON.stringify(args.slice(1)))) + "')");
            });
        }
    },
    ...
    add: function (action, webview) {
        if (action) {
            ...
            if (!_handlers[action.event].some(function (handlerWebView) {
                    return handlerWebView.id === webview.id;
                })) {
                _handlers[action.event].push(webview);
            }
