            ...
            if (data) {
                ...
                if (eventObj) {
                    ...
                    for (i = 0; i < fieldNameArr.length; i++) {
                        ...
                        if (eventObj[fieldNameArr[i].eventName]) {
                            if (!readObj) {
                                readObj = eventDetailsObj.ppsUtils.read();
                            }
                            //Checking if need to trigger the callback on the change
                            if (!fieldNameArr[i].skipTrigger || !fieldNameArr[i].skipTrigger(fieldNameArr[i].formatValue(readObj[fieldNameArr[i].eventName]))) {
                                returnObj = {};
                                break;
                            }
                        }
