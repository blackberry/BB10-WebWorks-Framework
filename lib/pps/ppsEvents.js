...
function onChangeMethod(eventMapObj, eventDetailsObj, trigger) {
    // Stores the value of field at the event registration time before any change occurred, so it can be compared latter.
    eventDetailsObj.fieldNameArr.forEach(function (fieldNameObj) {
        var readObj;
        if (fieldNameObj.setValue) {
            readObj = eventDetailsObj.ppsUtils.read();
            if (readObj) {
                fieldNameObj.setValue(readObj[fieldNameObj.eventName]);
            }
        }
    });
