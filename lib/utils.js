...
module.exports = {
    ...
    isLocalURI : function (uri) {
        return !!uri && !!uri.scheme && LOCAL_PROTOCOL.indexOf(uri.scheme) !== -1;
    },

    isFileURI : function (uri) {
        return !!uri && !!uri.scheme && FILE_PROTOCOL.indexOf(uri.scheme) !== -1;
    },

    isHttpURI : function (uri) {
        return !!uri && !!uri.scheme && HTTP_PROTOCOL.indexOf(uri.scheme) !== -1;
    },

    isHttpsURI : function (uri) {
        return !!uri && !!uri.scheme && HTTPS_PROTOCOL.indexOf(uri.scheme) !== -1;
    },

    // Checks if the specified uri starts with 'data:'
    isDataURI : function (uri) {
        return !!uri && !!uri.scheme && DATA_URI.indexOf(uri.scheme) !== -1;
    }
};
